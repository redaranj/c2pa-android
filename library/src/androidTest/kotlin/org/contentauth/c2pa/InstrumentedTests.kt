...
class InstrumentedTests {
    ...
    fun testBuilderAPI() {
        ...
        try {
            ...
            val fileTest = File.createTempFile("c2pa-stream-api-test", ".jpg", context.cacheDir)
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    val outputFile = File.createTempFile("c2pa-archive-output", ".jpg", context.cacheDir)
    ...
    fun testReaderWithManifestData() {
        ...
        try {
            val sourceImageData = getResourceAsBytes(R_RAW_PEXELS_ASADPHOTO_457882)
            val sourceStream = MemoryStream(sourceImageData)
            val fileTest = File.createTempFile("c2pa-manifest-direct-sign", ".jpg", context.cacheDir)
            val destStream = FileStream(fileTest, FileStream.Mode.WRITE)
    ...
    fun testSignerWithCallback() {
        ...
        try {
            val sourceImageData = getResourceAsBytes(R_RAW_PEXELS_ASADPHOTO_457882)
            val sourceStream = MemoryStream(sourceImageData)
            val fileTest = File.createTempFile("c2pa-callback-signer", ".jpg", context.cacheDir)
            val destStream = FileStream(fileTest, FileStream.Mode.WRITE)
    ...
    fun testWebServiceRealSigningAndVerification() {
        ...
        try {
            ...
            try {
                val sourceImageData = getResourceAsBytes(R_RAW_PEXELS_ASADPHOTO_457882)
                val sourceStream = MemoryStream(sourceImageData)
                val fileTest = File.createTempFile("c2pa-web-service-sign", ".jpg", context.cacheDir)
                val destStream = FileStream(fileTest, FileStream.Mode.WRITE)
                ...
                SigningAlgorithm.ES256, SigningAlgorithm.ES384, SigningAlgorithm.ES512 -> {
                    ...
                    if (algorithm == SigningAlgorithm.ES256) {
                        ...
                        try {
                            ...
                            val sourceImageData = getResourceAsBytes(R_RAW_PEXELS_ASADPHOTO_457882)
                            val sourceStream = MemoryStream(sourceImageData)
                            val fileTest = File.createTempFile("c2pa-algorithm-${algorithm.name}", ".jpg", context.cacheDir)
                            val destStream = FileStream(fileTest, FileStream.Mode.WRITE)
    ...
    fun testAlgorithmCoverage() {
        ...
        for (alg in algorithms) {
            ...
            // Try to create a signer with test keys to test algorithm support
            val testCertificate = "-----BEGIN CERTIFICATE-----\nDUMMY\n-----END CERTIFICATE-----"
            val testPrivateKey = "-----BEGIN PRIVATE KEY-----\nDUMMY\n-----END PRIVATE KEY-----"
            
            try {
                // This will fail but tells us if the algorithm is recognized
                val signer = Signer.fromInfo(SignerInfo(alg, testCertificate, testPrivateKey))
                signer.close()
