...
class CertificateSigningService {
    ...
    init {
        ...
        val rootSubject = X500Name("CN=C2PA Test Root CA, O=C2PA Signing Server, OU=Certificate Authority, C=US, ST=California, L=San Francisco")
        rootCA = generateCertificate(
            subject = rootSubject,
            issuer = rootSubject,
        ...
        val intermediateSubject = X500Name("CN=C2PA Test Intermediate CA, O=C2PA Signing Server, OU=Certificate Authority, C=US, ST=California, L=San Francisco")
        intermediateCA = generateCertificate(
    ...
    fun generateTemporaryCertificate(): Pair<String, PrivateKey> {
        ...
        val subject = X500Name("CN=Temporary C2PA Signer, O=Temporary Certificate, OU=FOR TESTING ONLY, C=US")
        val certificate = generateCertificate(
            subject = subject,
            issuer = X500Name(intermediateCA.subjectX500Principal.name),
    ...
    private fun generateCertificate(
        ...
        val now = Date()
        val notBefore = Date(now.time - 60000) // 1 minute ago
    ...
    private fun parseCertificateSigningRequest(csrPEM: String): PKCS10CertificationRequest {
        ...
        require(pemObject.type == "CERTIFICATE REQUEST") {
            "Invalid CSR format"
        }
