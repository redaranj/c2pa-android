...
class C2PAConfigurationController {
    suspend fun getConfiguration(call: ApplicationCall) {
        try {
            ...
            val encodedCertChain = Base64.getEncoder().encodeToString(certificateChain.toByteArray())

            val serverURL = System.getenv("SIGNING_SERVER_URL")
            if (serverURL.isNullOrEmpty()) {
                call.application.log.error("SIGNING_SERVER_URL environment variable is not set or is empty")
                call.respond(
                    HttpStatusCode.InternalServerError,
                    mapOf("error" to "SIGNING_SERVER_URL environment variable is not set or is empty")
                )
                return
            }
            ...
            val configuration = C2PAConfiguration(
                algorithm = "es256",
